...
impl TempStoreIntegration {
    pub async fn update_or_create(info: Value, db_pool: &Data<DbPool>) -> Result<Self, AppError> {
        let connection = &mut get_db_connection(db_pool)?;

        let _user = find_or_create_user(&info, db_pool).await?;

        let platform_id = IntegrationPlatform::find_by_name("salla".to_string(), db_pool).await?;
        let new_data = parse_store_data(info, platform_id.id);
    ...
    match User::find_by_email(email, db_pool).await {
        ...
        Err(_) => {
            let new_user = NewUser {
                first_name: info["user_name"].as_str().unwrap_or("").to_string(),
                last_name: info["user_name"].as_str().unwrap_or("").to_string(),
                email: email.to_string(),
                company_name: info["store_name"].as_str().unwrap_or("").to_string(),
                store_url: info["store_url"].as_str().unwrap_or("").to_string(),
                birth_day: Utc::now(),
...
fn parse_store_data(info: Value, platform_id: i64) -> NewTempStoreIntegration {
    NewTempStoreIntegration {
        shop_id: info["shop_id"].as_i64().unwrap_or(0) as i32,
        integration_platform_id: platform_id,
        store_name: info["store_name"].as_str().unwrap_or("").to_string(),
        store_url: info["store_url"].as_str().unwrap_or("").to_string(),
        email: info["email"].as_str().unwrap_or("").to_string(),
        auth_code: info["auth_code"].as_str().map(|s| s.to_string()),
        access_token: info["access_token"].as_str().unwrap_or("").to_string(),
        refresh_token: info["refresh_token"].as_str().unwrap_or("").to_string(),
        authorization_code: info["authorization_code"].as_str().map(|s| s.to_string()),
        expires: info["expires"].as_i64().unwrap_or(0),
...
fn insert_or_update_store(
    ...
    diesel::insert_into(temp_stores_integrations)
        .values(&new_data)
